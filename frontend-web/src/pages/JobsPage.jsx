...
function sortJobsCompanyAsc(list) {
  const copy = [...list];
  copy.sort((a, b) =>
...
function sortJobsStatusAsc(list) {
  const copy = [...list];
  copy.sort((a, b) =>
...
function hasOpenModal() {
  return Boolean(
    document.querySelector(
      'dialog[open], [role="dialog"], [aria-modal="true"], [data-modal="true"]'
    )
  );
}
...
export default function JobsPage() {
  ...
  // Allow effects to call the latest refreshJobs without re-subscribing.
  refreshJobsRef.current = refreshJobs;

  // Auto-refresh (paused when tab is not visible)
  useEffect(() => {
  ...
  async function refreshJobs({ source } = {}) {
    setError("");
    setLoadingJobs(true);
    ...
    try {
      ...
      if (refreshSeqRef.current !== mySeq) return; // stale response
      setJobs(sortJobsDesc(list));
      setLastUpdatedAt(new Date());

      // If the selected job no longer exists, clear details
  ...
  const filteredSortedJobs = useMemo(() => {
    ...
    return sortJobsDesc(filtered); // default
  }, [jobs, search, sortMode]);

  const visibleJobs = useMemo(
    () => filteredSortedJobs.slice(0, visibleCount),
    [filteredSortedJobs, visibleCount]
  );
  ...
  async function selectJob(job) {
    ...
    try {
      ...
      if (selectSeqRef.current !== mySeq) return; // stale response
      ...
      setNotes(sortNotesDesc(Array.isArray(freshNotes) ? freshNotes : []));
    } catch (e) {
  ...
  async function handleCreateJob(e) {
    ...
    try {
      ...
      setError(e2?.message ?? "Failed to create job");
    }
  }
  ...
  async function handleAddNote() {
    ...
    try {
      ...
      bumpSelectedJobActivity(created.created_at);
    } catch (e) {
  ...
  async function handleDeleteNote(noteId) {
    ...
    try {
      ...
      // re-fetch notes + job (server recalculates last_activity_at)
  ...
  function bumpSelectedJobActivity(iso) {
    if (!selectedJob) return;
    ...
    setJobs((prev) => {
      const next = prev.map((j) =>
        j.id === selectedJob.id ? { ...j, last_activity_at: iso } : j
      );
      return sortJobsDesc(next);
    });
  }
      ...
      {error && (
        <div className="mb-6 rounded-lg border border-red-900/60 bg-red-950/30 px-3 py-2 text-sm text-red-200">
          ...
          <div className="mt-2 text-xs text-slate-300">
            If this happened right after login, your session may have expired — try logging
            in again.
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="mb-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="mt-1 w-full rounded-lg border border-slate-800 bg-slate-950/40 px-3 py-2 text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-600/30"
              >
            ...
            <>
              ...
              <div className="mt-3 flex items-center justify-between gap-3">
                <div className="text-xs text-slate-500">
                  Showing{" "}
                  <span className="text-slate-300 font-medium">{visibleJobs.length}</span> of{" "}
        ...
        <div className="lg:col-span-2">
          ...
          {!selectedJob && !loadingDetails && (
            <div className="text-slate-400">Select a job to view details</div>
          )}

          {loadingDetails && (
            <div className="text-sm text-slate-400">Loading job details…</div>
          )}
          ...
          {selectedJob && !loadingDetails && (
            <div className="space-y-6">
              ...
              <DocumentsPanel
                ...
                onActivityChange={bumpSelectedJobActivity}
              />
            </div>
          )}
        </div>

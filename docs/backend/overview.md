...
- Environment variables (JWT secret, database credentials, email/GuardDuty toggles, AI config, etc.) are sourced from a single **AWS Secrets Manager** JSON bundle referenced by `SETTINGS_BUNDLE_SECRET_ARN`. The backend loads `.env`, fetches the bundle, hydrates `os.environ`, and then instantiates the normal settings object, so App Runner only needs one env var while local dev can reuse the exact same secret ARN.
...
- **CI/CD:** `.github/workflows/backend-deploy.yml` runs on pushes to `main`, assumes an AWS role via GitHub OIDC, builds/pushes the image, and executes `scripts/deploy_apprunner.py` twice (API service first, Celery worker second) to keep both App Runner services on the same tag. Each deployment waits for health and rolls back automatically if a deployment fails. Manual runs are available through `workflow_dispatch`.

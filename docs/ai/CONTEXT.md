...
- Credits are the gating mechanism for future AI features. `GET /billing/credits/balance` always recalculates from `credit_ledger`, `spend_credits/require_credits` lock the user row + enforce “no negative balance” writes, and every spend (usage, promo redemptions, etc.) must supply a per-user `idempotency_key`. A dev-only `/billing/credits/debug/spend` endpoint exists for local smoke tests (disabled in prod unless explicitly enabled).

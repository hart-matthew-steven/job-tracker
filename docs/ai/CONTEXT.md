...
- Credits are the gating mechanism for future AI features. `GET /billing/credits/balance` always recalculates from `credit_ledger`, `reserve_credits` + `finalize_charge`/`refund_reservation` lock the user row and enforce atomic holds, and every step carries its own per-user `idempotency_key`. `/ai/demo` exercises the full reserve/finalize/refund dance locally until OpenAI routes plug in.
